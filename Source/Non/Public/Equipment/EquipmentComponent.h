...
DECLARE_DYNAMIC_MULTICAST_DELEGATE_TwoParams(FOnEquipped, EEquipmentSlot, Slot,
                                             UInventoryItem *, Item);
DECLARE_DYNAMIC_MULTICAST_DELEGATE_OneParam(FOnUnequipped, EEquipmentSlot,
                                            Slot);
...
struct FEquipmentVisual {
  ...
  // 기본적으로 아이템 Row(=FItemData)의 AttachSocket을 쓰되, 슬롯별로 덮어쓸 수
  // 있게 함
...
public:
  ...
  // 인벤토리에서 SlotIndex 아이템을 꺼내 장비 슬롯에 장착 (OptionalTarget이
  // None이면 아이템의 Row.EquipSlot 사용)
  UFUNCTION(BlueprintCallable, Category = "Equipment")
  bool EquipFromInventory(UInventoryComponent *Inv, int32 FromIndex,
                          EEquipmentSlot OptionalTarget = EEquipmentSlot::None);
  ...
  UFUNCTION(BlueprintCallable, Category = "Equipment")
  bool EquipItem(UInventoryItem *Item,
                 EEquipmentSlot OptionalTarget = EEquipmentSlot::None);
  ...
  void ServerEquipFromInventory(
      int32 InvSlotIndex, EEquipmentSlot OptionalTarget = EEquipmentSlot::None);
  ...
  UFUNCTION(BlueprintCallable, Category = "Equipment|Visual",
            meta = (AutoCreateRefTerm = "Relative"))
  bool ReattachSlotToSocket(EEquipmentSlot Slot, FName NewSocket,
                            const FTransform &Relative);

  /** 트레이스 등에 쓰기 위해, 슬롯의 실제 UMeshComponent를 가져온다(없으면
   * nullptr) */
  UFUNCTION(BlueprintPure, Category = "Equipment|Visual")
  UMeshComponent *GetVisualComponent(EEquipmentSlot Slot) const;

  // [New] 슬롯에 액터 기반 스폰을 한 경우 해당 액터를 반환
  UFUNCTION(BlueprintPure, Category = "Equipment|Visual")
  ...
  void ClientSyncWeaponTags(const FGameplayTagContainer &TagsToAdd,
...
protected:
  ...
  // 내부 장착/해제
  bool EquipInternal(UInventoryItem *Item, EEquipmentSlot TargetSlot,
...
private:
  ...
  // [New] 슬롯 → 스폰된 무기/장비 액터
  UPROPERTY(Transient)
  TMap<EEquipmentSlot, TObjectPtr<AActor>> SpawnedEquipActors;
...
public:
  ...
  void
  RestoreEquippedItemsFromSave(const TArray<struct FEquipmentSaveData> &InData);
};
