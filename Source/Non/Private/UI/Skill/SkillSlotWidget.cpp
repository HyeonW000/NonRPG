...
void USkillSlotWidget::NativeOnInitialized() {
  ...
  if (Btn_LevelUp) {
    ...
    Btn_LevelUp->OnClicked.AddDynamic(this,
                                      &USkillSlotWidget::OnClicked_LevelUp);
  }
...
void USkillSlotWidget::SetupSlot(const FSkillRow &InRow,
                                 USkillManagerComponent *InMgr) {
  // 이전 바인딩 해제
  if (SkillMgr) {
    SkillMgr->OnSkillCooldownStarted.RemoveDynamic(
        this, &USkillSlotWidget::OnSkillCooldownStarted);
  }
  ...
  // 새 바인딩
  if (SkillMgr) {
    SkillMgr->OnSkillCooldownStarted.AddUniqueDynamic(
        this, &USkillSlotWidget::OnSkillCooldownStarted);
  }
...
void USkillSlotWidget::Refresh() {
  ...
  if (bHasMgr) {
    ...
    if (Text_Level) {
      Text_Level->SetText(FText::FromString(
          FString::Printf(TEXT("Lv %d / %d"), Lvl, Row.MaxLevel)));
    }
    ...
    if (Btn_LevelUp) {
      ...
      Btn_LevelUp->SetVisibility(bShouldShowButton
    ...
    if (Text_Level) {
      Text_Level->SetText(FText::FromString(
          FString::Printf(TEXT("Lv %d / %d"), 0, Row.MaxLevel)));
    }
  ...
  if (LockOverlay) {
    LockOverlay->SetVisibility(bLocked ? ESlateVisibility::HitTestInvisible
                                       : ESlateVisibility::Collapsed);
  }
  ...
  if (IconImage) {
    if (Row.Icon.IsNull()) {
      ...
      if (UTexture2D *Tex = Row.Icon.Get()) {
        ...
        SM.RequestAsyncLoad(Row.Icon.ToSoftObjectPath(),
...
// Drag Drop 및 클릭 포커스 처리
FReply
USkillSlotWidget::NativeOnMouseButtonDown(const FGeometry &InGeometry,
  ...
  if (InMouseEvent.GetEffectingButton() == EKeys::LeftMouseButton) {
    ...
    if (TSharedPtr<SViewport> VP =
            UIViewportUtils::GetGameViewportSViewport(GetWorld())) {
      Reply = Reply.SetUserFocus(StaticCastSharedRef<SWidget>(VP.ToSharedRef()),
                                 EFocusCause::SetDirectly);
      FSlateApplication::Get().SetKeyboardFocus(
          StaticCastSharedPtr<SWidget>(VP), EFocusCause::SetDirectly);
    }
...
void USkillSlotWidget::NativeOnDragDetected(const FGeometry &InGeometry,
                                            const FPointerEvent &InMouseEvent,
...
// Cooldown Logic
// --------------------------------------------------------------------------------------
void USkillSlotWidget::OnSkillCooldownStarted(FName InSkillId, float Duration,
                                              float EndTime) {
...
void USkillSlotWidget::StartCooldown(float Duration, float EndTime) {
  ...
  if (UWorld *World = GetWorld()) {
    World->GetTimerManager().SetTimer(CooldownTimerHandle, this,
                                      &USkillSlotWidget::UpdateCooldownTick,
                                      0.05f, true);
  }

  if (ImgCooldownRadial)
    ImgCooldownRadial->SetVisibility(ESlateVisibility::SelfHitTestInvisible);
  UpdateCooldownTick();
}
...
void USkillSlotWidget::UpdateCooldownTick() {
  ...
  if (TxtCooldown) {
    ...
    if (Seconds > 0) {
      TxtCooldown->SetText(
          FText::FromString(FString::Printf(TEXT("%ds"), Seconds)));
