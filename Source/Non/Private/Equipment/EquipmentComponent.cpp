...
static FName GetDefaultSocketForSlot(EEquipmentSlot Slot) {
  switch (Slot) {
  case EEquipmentSlot::WeaponMain:
    return FName(TEXT("hand_r_socket"));
  case EEquipmentSlot::WeaponSub:
    return FName(TEXT("hand_l_socket"));
  case EEquipmentSlot::Head:
    return FName(TEXT("head_socket"));
  case EEquipmentSlot::Chest:
    return FName(TEXT("chest_socket"));
  case EEquipmentSlot::Hands:
    return FName(TEXT("hand_r_socket")); // 필요 시 변경
  case EEquipmentSlot::Feet:
    return FName(TEXT("foot_r_socket")); // 필요 시 변경
  default:
    return NAME_None;
  }
}
...
UEquipmentComponent::UEquipmentComponent() {
  ...
  SetIsReplicatedByDefault(
      true); // [Multiplayer] 컴포넌트 리플리케이션 활성화 (필수)
}

void UEquipmentComponent::BeginPlay() {
  Super::BeginPlay();
  OwnerInventory = GetOwner()
...
USkeletalMeshComponent *UEquipmentComponent::GetOwnerMesh() const {
  ...
  return GetOwner() ? GetOwner()->FindComponentByClass<USkeletalMeshComponent>()
                    : nullptr;
}

bool UEquipmentComponent::EquipFromInventory(UInventoryComponent *Inv,
                                             int32 FromIndex,
                                             EEquipmentSlot OptionalTarget) {
  if (!Inv)
    return false;
  ...
  const EEquipmentSlot Target =
      (OptionalTarget != EEquipmentSlot::None) ? OptionalTarget : Row.EquipSlot;

  FText Fail;
  if (!CanEquip(Item, Target, Fail))
    return false;

  // (A) 먼저 한 칸 확보: FromIndex를 비워둔다.
  Inv->RemoveAt(FromIndex, 1);

  // (B) 장착 시도 (교체품은 EquipInternal 내부에서 OwnerInventory->AddItem으로
  // 되돌아감)
  ...
  // (D) 교체품이 인벤토리 어딘가(PutIdx)에 들어갔다면 → 사용자가 뺀
  // 자리(FromIndex)로 정렬
...
bool UEquipmentComponent::EquipItem(UInventoryItem *Item,
                                    EEquipmentSlot OptionalTarget) {
  if (!Item)
    return false;
  const FItemRow &Row = Item->CachedRow;
...
bool UEquipmentComponent::UnequipToInventory(EEquipmentSlot Slot,
  ...
  // 인벤이 없으면 일단 그냥 해제(아이템 유실 방지 목적이면 false를 리턴하는
  // 선택지도 가능)
  if (!OwnerInventory) {
  ...
  int32 PutIdx = INDEX_NONE;
  const bool bAdded = OwnerInventory->AddItem(
      Cur->ItemId, FMath::Max(1, Cur->Quantity), PutIdx);
  if (!bAdded) {
...
UInventoryItem *
UEquipmentComponent::GetEquippedItemBySlot(EEquipmentSlot Slot) const {
...
bool UEquipmentComponent::ReattachSlotToSocket(EEquipmentSlot Slot,
                                               FName NewSocket,
...
UMeshComponent *
UEquipmentComponent::GetVisualComponent(EEquipmentSlot Slot) const {
...
bool UEquipmentComponent::CanEquip(const UInventoryItem *Item,
                                   EEquipmentSlot TargetSlot,
                                   FText &OutFailReason) const {
...
bool UEquipmentComponent::EquipInternal(UInventoryItem *Item,
                                        EEquipmentSlot TargetSlot,
                                        int32 *OutReturnedIndex /*=nullptr*/) {
  ...
  if (TObjectPtr<UInventoryItem> *FoundPtr = Equipped.Find(TargetSlot)) {
    ...
    if (FGrantedAbilityHandles *HandleEntry =
            GrantedAbilityHandles.Find(TargetSlot)) {
  ...
  if (GetOwner()->HasAuthority()) {
    ...
    ReplicatedItems.RemoveAll(
        [TargetSlot](const FReplicatedEquipmentItem &Entry) {
  ...
  auto IsTwoHandOrStaff = [](const UInventoryItem *It) -> bool {
    return It && (It->IsTwoHandedWeapon() ||
                  It->CachedRow.WeaponType ==
                      EWeaponType::Staff); // 프로젝트에 맞게 Staff 판정 보강
  };

  if (TargetSlot == EEquipmentSlot::WeaponSub) {
    // 서브(방패) 장착 시도 → 메인에 2H/Staff 있으면 메인을 해제하여 "서브
    // 장착을 우선"
    if (UInventoryItem *Main =
            GetEquippedItemBySlot(EEquipmentSlot::WeaponMain)) {
    ...
    // 메인 장착 시도 → 새 메인이 2H/Staff라면 서브(방패)를 해제하여 "메인
    // 장착을 우선"
  ...
  if (bStillEquippedHere) {
    ...
    // [New] 어빌리티 부여 (GrantedAbilities) - 서버에서만 수행 (GAS
    // 리플리케이션)
    if (GetOwner()->HasAuthority()) {
      if (ANonCharacterBase *Char = Cast<ANonCharacterBase>(GetOwner())) {
        if (UAbilitySystemComponent *ASC = Char->GetAbilitySystemComponent()) {
          ...
          if (Item->CachedRow.GrantedAbilities.Num() > 0) {
            FGrantedAbilityHandles &HandleEntry =
                GrantedAbilityHandles.FindOrAdd(TargetSlot);

            for (const TSubclassOf<UGameplayAbility> &AbilityClass :
                 Item->CachedRow.GrantedAbilities) {
              if (AbilityClass) {
          ...
          if (const FWeaponDefaultAbilities *FoundSet =
                  WeaponTypeDefaultAbilities.Find(Item->CachedRow.WeaponType)) {
            for (const TSubclassOf<UGameplayAbility> &AbilityClass :
                 FoundSet->Abilities) {
              if (AbilityClass) {
                FGrantedAbilityHandles &HandleEntry =
    ...
    // 손 재부착/스탠스 갱신은 '메인 슬롯 장착' && '이전에 메인무기 있었음' &&
    // '지금도 Armed' 일 때만 손 재부착/스탠스 갱신은 '메인 슬롯 장착' &&
    // '이전에 메인무기 있었음' && '지금도 Armed' 일 때만 [수정] 방패(Sub) 장착
    // 시에도 즉시 Armed 처리 및 스탠스 갱신 (토글 없음)
    if (ANonCharacterBase *Char = Cast<ANonCharacterBase>(GetOwner())) {
      ...
      // {
      //    // [Legacy] 방패 장착 시 Shield 스탠스 때문에 강제 Armed 했던 코드
      //    -> 삭제
      //    // Char->SetArmed(true);
      // }
    }
  ...
  if (Replaced) {
    if (OwnerInventory) {
      ...
      const bool bReturned =
          OwnerInventory->AddItem(ReplacedId, ReplacedQty, ReturnedIdx);
      if (!bReturned) {
        // 되돌리기 실패 → 전체 롤백
        if (bStillEquippedHere) {
        ...
        RecomputeSetBonuses();
        if (OutReturnedIndex) {
          *OutReturnedIndex = INDEX_NONE;
        }
        return false;
      }

      if (OutReturnedIndex) {
        *OutReturnedIndex = ReturnedIdx;
      }
    } else {
      if (OutReturnedIndex) {
        *OutReturnedIndex = INDEX_NONE;
...
void UEquipmentComponent::UnequipInternal(EEquipmentSlot Slot) {
  ...
  if (GetOwner()->HasAuthority()) {
    ReplicatedItems.RemoveAll([Slot](const FReplicatedEquipmentItem &Entry) {
...
void UEquipmentComponent::ResolveTwoHandedConflicts() {
  ...
  if (UInventoryItem *Main =
          GetEquippedItemBySlot(EEquipmentSlot::WeaponMain)) {
    const bool bTwoHandOrStaff = Main->IsTwoHandedWeapon() ||
                                 (Main->CachedRow.WeaponType ==
                                  EWeaponType::Staff); // 프로젝트에 맞게 조정
...
void UEquipmentComponent::ResolveWeaponSubConflicts() {
  ...
  if (UInventoryItem *Sub = GetEquippedItemBySlot(EEquipmentSlot::WeaponSub)) {
    const bool bWeaponSub =
        (Sub->CachedRow.WeaponType == EWeaponType::WeaponSub);
    if (!bWeaponSub)
      return;
...
void UEquipmentComponent::ApplyVisual(EEquipmentSlot Slot,
                                      const FItemRow &Row) //  FItemRow
{
  RemoveVisual(Slot);
  ...
  if (const FEquipmentVisual *VS = VisualSlots.Find(Slot)) {
    if (VS->Socket != NAME_None)
      SocketToUse = VS->Socket;
...
// ========= 소켓 관련=======
void UEquipmentComponent::SetHomeSocketForSlot(EEquipmentSlot Slot,
                                               FName SocketName) {
...
void UEquipmentComponent::RecomputeHomeSocketFromEquipped(EEquipmentSlot Slot) {
  ...
  const FName Fallback =
      bTwoHand ? DefaultSheathSocket2H : DefaultSheathSocket1H;
...
USceneComponent *
UEquipmentComponent::GetVisualForSlot(EEquipmentSlot Slot) const {
  if (const TObjectPtr<UMeshComponent> *Found = VisualComponents.Find(Slot)) {
...
bool UEquipmentComponent::IsTwoHandedLike(const UInventoryItem *Item) const {
  ...
  const EWeaponType WT =
      Item->CachedRow.WeaponType; // 예: Item->CachedRow.WeaponType;
  switch (WT) {
...
bool UEquipmentComponent::IsMainHandOccupyingBothHands() const {
  const UInventoryItem *Main =
      GetEquippedItemBySlot(EEquipmentSlot::WeaponMain);
  return IsTwoHandedLike(Main);
}
#include "System/NonSaveGame.h"

TArray<FEquipmentSaveData>
UEquipmentComponent::GetEquippedItemsForSave() const {
...
void UEquipmentComponent::RestoreEquippedItemsFromSave(
    const TArray<FEquipmentSaveData> &InData) {
  // 기존 장비 해제
  ...
  for (const FEquipmentSaveData &Data : InData) {
    ...
    // 일단 GameInstance의 Subsystem(가령 SkillSystemSubsystem 처럼
    // ItemDataSubsystem이 있는지?) 또는 AssetManager? NonCharacterBase에서
    // 처리하는게 편할 수도 있지만 여기서는 직접 처리. 임시:
    // InventoryComponent가 있다면 거기서 로딩 위임 (MakeItem 같은게 있다면).
    ...
    // 해결책: SaveGameSubsystem에서 로드할 때 InventoryComponent가 이미
    // 로드되어 있음. InventoryComponent 쪽에 'CreateItem(ItemId)' 헬퍼가 있는게
    // 베스트. 없으면.. 데이터 테이블을 직접 로드해야 함. 일단 여기서는 "Row
    // 로드" 로직이 필요함을 인지.
    ...
    // NonGameSettings에 MasterSkillTable만 있었음. ItemTable은?
    // InventoryComponent가 들고 있을 확률 높음.
    ...
    // -> 유저가 "비주얼 안나옴" 하면 그때 고치자? 안됨. "장착 아이템 사라짐" ==
    // 비주얼 안나옴 일수도.
    ...
    // 일단 시도: OwnerInventory->GetItemData(ItemId) (존재한다고 가정하거나,
    // 새로 만들어야 함)
    // -> 이미 빌드된 코드라 함수 없을 확률 99%.

    // 대안: SaveGameSubsystem에서 복구 할 때, ItemTable에 접근 권한이 있을 수
    // 있음. 하지만 여기서 처리하는게 깔끔.

    // *우회*: EquipInternal 호출 후, ApplyVisual이 있는데, 여기서 Row를 씀.

    // [해결책] "TryGetItemRow" 함수가 InventoryComponent에 있다고 믿거나
    // 추가해야 함.
    // -> InventoryComponent.cpp를 보면 FindRow 하는 로직이 어딘가에 있음.
    // -> AddItem(ItemId) -> ... -> FindRow.

    // 그렇다면, 잠시 인벤토리에 AddItem 했다가 -> 그걸 집어서
    // EquipFromInventory -> 다시 인벤토리에서 제거? 약간 비효율적이지만 가장
    // 안전함 (Row 로드 로직 재사용).
...
void UEquipmentComponent::UpdateWeaponTags(bool bIsArmed) {
  ...
  if (UInventoryItem *Sub = GetEquippedItemBySlot(EEquipmentSlot::WeaponSub)) {
    if (const FGameplayTagContainer *FoundTags =
            WeaponTypeArmedTags.Find(Sub->CachedRow.WeaponType)) {
  ...
  if (UInventoryItem *Main =
          GetEquippedItemBySlot(EEquipmentSlot::WeaponMain)) {
    if (const FGameplayTagContainer *FoundTags =
            WeaponTypeArmedTags.Find(Main->CachedRow.WeaponType)) {
      if (FoundTags->Num() > 0) {
        ASC->AddLooseGameplayTags(*FoundTags);
      }
    }
  ...
  // 여기서는 함수 구조상 전체 로직을 재현하기보다, 결과적으로 가져야 할 태그를
  // 다시 계산해서 보내는 것이 안전.
  ...
  // 따라서 클라이언트에게 "이것들을 지우고, 이것을 추가해라" 라고 명확히 보내야
  // 함.
...
void UEquipmentComponent::ClientSyncWeaponTags_Implementation(
...
void UEquipmentComponent::OnRep_ReplicatedItems() {
  ...
  for (EEquipmentSlot Slot : LocalSlots) {
    const bool bServerHas = ReplicatedItems.ContainsByPredicate(
        [Slot](const FReplicatedEquipmentItem &Entry) {
          return Entry.Slot == Slot;
        });
  ...
  UInventoryComponent *InvComp =
      GetOwner() ? GetOwner()->FindComponentByClass<UInventoryComponent>()
                 : nullptr;
  if (!InvComp) {
  ...
  for (const FReplicatedEquipmentItem &Entry : ReplicatedItems) {
    ...
    // 로컬 장착 처리 (비주얼 생성)
    // EquipInternal을 쓰되, [서버] 로직인 ReplicatedItems.Add는 막아야 함 ->
    // Authority 체크가 있으니 OK
    EquipInternal(NewItem, Entry.Slot);
  }
}

void UEquipmentComponent::ServerEquipFromInventory_Implementation(
    int32 InvSlotIndex, EEquipmentSlot OptionalTarget) {
  if (!OwnerInventory)
    OwnerInventory =
        GetOwner() ? GetOwner()->FindComponentByClass<UInventoryComponent>()
                   : nullptr;
