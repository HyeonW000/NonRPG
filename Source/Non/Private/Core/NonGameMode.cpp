    ...
    if (!MapName.Contains(TEXT("Lobby")) && !MapName.Contains(TEXT("Title")))
    {
        ...
        if (ANonPlayerController* NonPC = Cast<ANonPlayerController>(NewPlayer))
        {
            if (NonPC->SelectedSlotIndex >= 0)
            {
                ...
                UE_LOG(LogTemp, Log, TEXT("[NonGM] PostLogin: Provide Slot Index %d from Controller (URL Option)."), TargetSlotIndex);
            }
        }
            ...
            if (UNonGameInstance* GI = Cast<UNonGameInstance>(GetGameInstance()))
            {
                if (GI->CurrentSlotIndex >= 0)
                {
                    TargetSlotIndex = GI->CurrentSlotIndex;
                     UE_LOG(LogTemp, Log, TEXT("[NonGM] PostLogin: Provide Slot Index %d from GameInstance."), TargetSlotIndex);
                }
            }
        }
...
void ANonGameMode::SpawnPlayerFromSave(APlayerController* PC, int32 SlotIndex)
{
    ...
    if (UGameplayStatics::DoesSaveGameExist(SlotName, 0))
    {
        ...
        UE_LOG(LogTemp, Log, TEXT("[NonGM] SpawnPlayerFromSave: Loaded Saved Data for %s"), *SlotName);
    }

    if (!Data)
    {
        UE_LOG(LogTemp, Warning, TEXT("[NonGM] SpawnPlayerFromSave: No SaveData for %s. Spawning Default."), *SlotName);
        if (GEngine) GEngine->AddOnScreenDebugMessage(-1, 5.f, FColor::Red, FString::Printf(TEXT("[NonGM] No SaveData for %s"), *SlotName));
    }
    ...
    if (GEngine) GEngine->AddOnScreenDebugMessage(-1, 5.f, FColor::Cyan, TEXT("[NonGM] Attempting to spawn character..."));
    ...
    if (!PawnClass) PawnClass = ANonCharacterBase::StaticClass();

    if (APawn* NewPawn = GetWorld()->SpawnActor<APawn>(PawnClass, SpawnTransform, SpawnParams))
    {
		...
		if (ANonCharacterBase* Char = Cast<ANonCharacterBase>(NewPawn))
		{
            ...
            {
                // 신규 캐릭터 기본값
                Char->InitCharacterData(EJobClass::Defender, 1);
...
// [New] 접속 시 URL 옵션 파싱
APlayerController* ANonGameMode::Login(UPlayer* NewPlayer, ENetRole InRemoteRole, const FString& Portal, const FString& Options, const FUniqueNetIdRepl& UniqueId, FString& ErrorMessage)
{
    ...
    if (PC)
    {
        ...
        if (!SlotStr.IsEmpty() && SlotStr.IsNumeric())
        {
            ...
            if (ANonPlayerController* NonPC = Cast<ANonPlayerController>(PC))
            {
                // SelectedSlotIndex는 Rep Notify가 있을 수 있으므로 서버에서 설정하면 복제됨
                NonPC->SelectedSlotIndex = SlotIndex;
                UE_LOG(LogTemp, Log, TEXT("[NonGM] Login: Parsed Slot Index %d from Options."), SlotIndex);
            }
        }
    }
